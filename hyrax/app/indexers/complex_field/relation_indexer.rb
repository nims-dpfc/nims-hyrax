module ComplexField
  module RelationIndexer
    ...
    def index_relation(solr_doc)
      ...
      solr_doc[Solrizer.solr_name('complex_relation_relationship', :facetable)] = object.complex_relation.map { |r| r.relationship_name.reject(&:blank?).first }
      object.complex_relation.each do |r|
        unless r.title.blank? || r.relationship_name.blank?
          relationship = r.relationship_name.reject(&:blank?).first.downcase.tr(' ', '_')
