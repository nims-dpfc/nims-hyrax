  ...
  module StructuralFeatureIndexer
    ...
    def index_structural_feature(solr_doc)
      object.complex_specimen_type.each do |st|
        # category as complex_structural_feature_category searchable
        vals = st.complex_structural_feature.map { |c| c.category.reject(&:blank?) }
        ...
        vals = st.complex_structural_feature.map { |c| c.description.reject(&:blank?) }
        ...
        vals = st.complex_structural_feature.map { |c| c.sub_category.reject(&:blank?) }
        ...
        solr_doc[fld_name].flatten!
        # identifier
        st.complex_structural_feature.each do |cc|
