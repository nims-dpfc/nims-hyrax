      ...
      class OpenUrlFormatter < BaseFormatter
        def format(work)
          ...
          FIELD_MAP.each do |element, kev|
            ...
            # ensure the abstract is not visible unless the user is authorised to view it
            next if element == :description && view_context.cannot?(:read_abstract, work.model_name.name.constantize)
