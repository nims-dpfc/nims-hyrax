    ...
    module Ga4
      ...
      class_methods do
        ...
        def default_date_range
          "#{format_date(Hyrax.config.analytics_start_date)},#{format_date(Time.zone.today + 1.day)}"
        end

        # The number of events by day for an action
        def daily_events(action, date = default_date_range)
          date = date.split(",")
          Hyrax::Analytics::Google::EventsDaily.summary(profile, format_date(date[0]), format_date(date[1]), action)
        end

        # The number of events by day for an action and ID
        def daily_events_for_id(id, action, date = default_date_range)
          date = date.split(",")
          Hyrax::Analytics::Google::EventsDaily.by_id(profile, format_date(date[0]), format_date(date[1]), id, action)
        end

        # A list of events sorted by highest event count
        def top_events(action, date = default_date_range)
          date = date.split(",")
          Hyrax::Analytics::Google::Events.send('list', profile, format_date(date[0]), format_date(date[1]), action)
        end
        ...
        def new_visitors(period = 'month', date = default_date_range)
          ...
          Hyrax::Analytics::Google::Visits.new_visits(profile, format_date(date[0]), format_date(date[1]))
        end

        def new_visits_by_day(date = default_date_range, _period = 'day')
          date = date.split(",")
          Hyrax::Analytics::Google::VisitsDaily.new_visits(profile, format_date(date[0]), format_date(date[1]))
        end

        def returning_visitors(period = 'month', date = default_date_range)
          date = date_period(period, date)
          Hyrax::Analytics::Google::Visits.return_visits(profile, format_date(date[0]), format_date(date[1]))
        end

        def returning_visits_by_day(date = default_date_range, _period = 'day')
          date = date.split(",")
          Hyrax::Analytics::Google::VisitsDaily.return_visits(profile, format_date(date[0]), format_date(date[1]))
        end

        def total_visitors(period = 'month', date = default_date_range)
          date = date_period(period, date)
          Hyrax::Analytics::Google::Visits.total_visits(profile, format_date(date[0]), format_date(date[1]))
        end

        def format_date(date)
          date.to_date.to_formatted_s('%Y-%m-%d')
        end
      end
