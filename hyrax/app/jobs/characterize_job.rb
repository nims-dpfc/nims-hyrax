...
class CharacterizeJob < Hyrax::ApplicationJob
  ...
  def characterize(file_set, _file_id, filepath) # rubocop:disable Metrics/AbcSize
    ...
    # TODO remove force_encoding('UTF-8')
    original_name = file_set.characterization_proxy.original_name.force_encoding('UTF-8')
  
    file_set.title = [original_name] if reset_title
    # always set the label to the original_name
    file_set.label = original_name
