...
Given(/^an? (guest|nims_researcher|external_researcher|admin) user$/) do |user_type|
...
Given(/^I am logged in as an? (guest|nims_researcher|external_researcher|admin) user$/) do |user_type|
  step "a #{user_type} user"
  step 'I am logged in'
end

Given(/^there (?:are|is) (\d+) (guest|nims_researcher|admin) users?$/) do |number, user_type|
  @users ||= {}
...
Then(/^I should see the (guest|nims_researcher|admin) users?$/) do |user_type|
...
Then(/^I should not see the (guest|nims_researcher|admin) users?$/) do |user_type|
  # first, verify @users is present and has some data
  expect(@users[user_type]).to be_present
