...
Given(/^an? (guest|email|nims_researcher|admin) user$/) do |user_type|
...
Given(/^I am logged in as an? (guest|email|nims_researcher|admin) user$/) do |user_type|
  step "a #{user_type} user"
  step 'I am logged in'
end

Given(/^there (?:are|is) (\d+) (guest|email|nims_researcher|admin) users?$/) do |number, user_type|
  @users ||= {}
...
Then(/^I should see the (guest|email|nims_researcher|admin) users?$/) do |user_type|
