...
RSpec.describe NestedCrystallographicStructureInput, type: :input do
  ...
  let(:value) { dataset.complex_specimen_type.first.complex_crystallographic_structure.first }
  let(:index) { 0 }
  let(:options) { {} }
  ...
  it 'generates the correct fields' do
    ...
    is_expected.not_to have_select('dataset[complex_crystallographic_structure_attributes][0]_complex_identifier_attributes_0_scheme', selected: 'Identifier - Persistent')
  end
end
