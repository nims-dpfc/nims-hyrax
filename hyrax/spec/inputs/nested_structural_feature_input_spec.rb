...
RSpec.describe NestedStructuralFeatureInput, type: :input do
  ...
  let(:dataset) { build(:dataset, :with_complex_structural_feature) }
  ...
  let(:value) { dataset.complex_structural_feature.first }
  let(:index) { 0 }
  let(:options) { {} }
  ...
  it 'generates the correct fields' do
    ...
    is_expected.not_to have_field('dataset_complex_structural_feature_attributes_0_sub_category', type: :text, with: 'structural feature sub category')
