...
RSpec.describe 'hyrax/publications/_attribute_rows' do
  ...
  let(:publication) { build(:publication, :with_alternative_title, :with_people, :with_keyword, :with_subject, :with_language,
  ...
  let(:presenter) { Hyrax::PublicationPresenter.new(SolrDocument.new(publication.to_solr), Ability.new(user)) }
  ...
  context 'unauthenticated user' do
    ...
    it 'shows the correct metadata' do
      expect(rendered).to have_content('Publication Alternative Title')
      ...
      expect(rendered).to have_content('Big Baz')
